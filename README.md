...
| [`modules/caps_lock.lua`](modules/caps_lock.lua) | **CAPS LOCK toggle alert.** `[⇪] ABC` with an orange badge for 2s when engaged, `[⇪] abc` in gray for 0.6s when released. Follows input_source's alert mode (hidden when `off`) |
